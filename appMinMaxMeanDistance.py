...
def aprendizadoAtivo(X_train, X_test, y_train, y_test):
    ...
    for i in range(0, kmeans.n_clusters):
        cluster_len = len(cluster_distance[i])
        if biggest_cluster_size < len(cluster_distance[i]):
            biggest_cluster_size = len(cluster_distance[i])
        
        aux_distance = np.argsort(cluster_distance[i])
        stop = int(cluster_len / 3)
        first_circle = aux_distance[:stop]
        start = stop
        stop = stop + stop
        third_circle = aux_distance[start:stop]
        second_circle = aux_distance[stop:]
        sorted_index.append(np.concatenate((first_circle, second_circle, third_circle), axis=0))
